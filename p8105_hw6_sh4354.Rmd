...
We create a simple linear regression model with `tmax` as the response and `tmin` and `prcp` as the predictors. Using `broom:glance`, we extract r^2 from the fitted regression. 
```{r}
model = 
  weather_df |> 
...
Next, we use `broom:tidy` to compute log(β̂1∗β̂2). If either `tmin` or `prcp` contain a negative value, the log value is recorded as "N/A."
```{r}
weather_df |> 
  lm(tmax ~ tmin + prcp, data = _) |> 
  broom::tidy() |> 
...
Using a function, we create a data frame that contains the r^2 and log(β̂1∗β̂) value from a simple linear regression model with `tmax` as the response and `tmin` and `prcp` as the predictors. 
...
Now we produce estimates of the quantities for each of 5000 bootstrap samples. 
```{r}
boot_straps = 
...
Finally, using the 5000 bootstrap estimates, we identify the 2.5% and 97.5% quantiles to provide a 95% confidence interval for r^2 and log(β̂1∗β̂). 
```{r}
boot_straps |> 
